...
METADATA_FLAG_NAMES = {
    ...
    "abstract_from_section_recovery",
...
def main() -> None:
    ...
    if compat_output:
        write_jsonl(compat_output, repaired_records)
    report = {
    ...
    print(f"[metadata-enrich] input={input_path}")
    print(f"[metadata-enrich] override={override_path}")
    print(f"[metadata-enrich] output={output_path}")
    if compat_output:
        print(f"[metadata-enrich] compat_output={compat_output}")
    print(
        "[metadata-enrich] missing_abstract "
        f"{report['missing_abstract_before']} -> {report['missing_abstract_after']}"
    )
    print(f"[metadata-enrich] override_applied_count={report['override_applied_count']}")
    print(
        "[metadata-enrich] section_abstract_recovery_count="
        f"{report['section_abstract_recovery_count']}"
    )
    print(f"[metadata-enrich] bad_fallback_title_count={report['bad_fallback_title_count']}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Enrich local S2ORC metadata before peS2o-style filtering.")
    ...
    parser.add_argument("--output", default="data/structured/local_s2orc_enriched.jsonl")
    parser.add_argument("--compat-output", default="data/structured/s2orc_like_repaired.jsonl")
    parser.add_argument("--report-md", default="outputs/reports/metadata_enrichment_report.md")
    parser.add_argument("--report-json", default="outputs/reports/metadata_enrichment_report.json")
...
def repair_record(record: dict, overrides: dict[str, dict]) -> tuple[dict, list[str]]:
    ...
    old_flags = list(record.get("quality_flags") or [])
    old_metadata_flags = list(record.get("metadata_flags") or [])
    structural_flags = list(record.get("structural_flags") or [])
    events: list[str] = []
    notes: list[str] = []
    ...
    metadata_flags = recompute_metadata_flags(
        record,
        title_source=metadata.get("title_source"),
        metadata_override_applied="metadata_override_csv" in events,
    )
    old_non_metadata_flags = [
        flag
        for flag in old_flags
        if flag not in METADATA_FLAG_NAMES and flag not in old_metadata_flags
    ]
    record["metadata_flags"] = unique_keep_order(old_metadata_flags + metadata_flags)
    record["quality_flags"] = unique_keep_order(
        structural_flags + old_non_metadata_flags + metadata_flags
    )
    record["metadata_enrichment"] = {
        "applied": bool(events),
...
def recompute_metadata_flags(
    ...
    if title_source == "filename_fallback":
        ...
        flags.append("title_needs_external_verification")
